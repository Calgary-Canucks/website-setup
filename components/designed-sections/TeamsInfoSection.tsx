...
const TeamsInfoSection: React.FunctionComponent<ITeamsInfoSectionProps> = ({
  ...
  if (dynamicsSportsTeams.length > 1) {
    groups = dynamicsSportsTeams.reduce((teams: any, item) => {
      const group = teams[item.bsi_AgeGroup.bsi_name] || [];
      group.push(item);
      teams[item.bsi_AgeGroup.bsi_name] = group;
      return teams;
    }, {});
  }
        ...
        <Box w="95%" mx="auto" py={8}>
          ...
          >
            ...
            >
              ...
              >
                {dynamicsSportsTeams[0].bsi_teaminfo}
              </Text>
              <Box
                fontSize="1rem"
                textAlign={{ base: "start", md: "start" }}
                dangerouslySetInnerHTML={{
                  __html: dynamicsSportsTeams[0].bsi_description,
                }}
              ></Box>
            </Flex>
            <Image
